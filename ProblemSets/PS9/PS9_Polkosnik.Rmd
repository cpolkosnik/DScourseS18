...
Part 4.
```{r}
# Loading in the housing data from UCI
...
mod_matrix <- data.frame(model.matrix(formula, housing)) #now replace the intercept column by the response since MLR will do "y ~ ." and get the intercept by default
...
Part 6.
```{r}
housing2 <- read.table("http://archive.ics.uci.edu/ml/machine-learning-databases/housing/housing.data")
names(housing2) <- c("crim","zn","indus","chas","nox","rm","age","dis","rad","tax","ptratio","b","lstat","medv")

# Add some other features
housing2$lmedv <- log(housing2$medv)
housing2$medv <- NULL
housing2$dis2 <- housing2$dis^2
housing2$chasNOX <- housing2$crim * housing2$nox

# Break up the data:
n <- nrow(housing2)
train <- sample(n, size = .8*n)
test  <- setdiff(1:n, train)
housing.train <- housing2[train,]
housing.test  <- housing2[test, ]
...
sqrt(mean((prediction$data$truth-prediction$data$response)^2)) # finding out of sample RMSE
```
